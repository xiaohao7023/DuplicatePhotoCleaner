...
struct DuplicateGroupDetailView: View {
    let group: DuplicateGroup; let groupIndex: Int; let totalGroups: Int
        ...
        ScrollView {
            VStack(spacing: 20) {
                ...
                HStack(alignment: .top, spacing: 16) {
                    // Left: Best photo
                    VStack(spacing: 8) {
                        BestPhotoCard(asset: bestAsset)
                        StatusTag(text: "Best Quality", type: .success)
                    }
                    .frame(maxWidth: .infinity)

                    // Right: Stacked duplicates
                    VStack(spacing: 8) {
                        ZStack {
                            ForEach(Array(otherAssets.prefix(5).enumerated()), id: \.element.localIdentifier) { index, asset in
                                ...
                                StackPhotoCard(asset: asset, isSelected: isSelected, index: index, total: min(otherAssets.count, 5))
                                    .onTapGesture {
...
private struct BestPhotoCard: View {
