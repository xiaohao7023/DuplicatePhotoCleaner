...
struct DuplicateGroupsView: View {
    ...
    private var visibleGroups: [DuplicateGroup] {
        groups.compactMap { group in
            ...
            return DuplicateGroup(assets: remaining, recommended: recommended)
        }
    }
                ...
                ForEach(Array(visibleGroups.enumerated()), id: \.element.assets.first?.localIdentifier) { index, group in
                    DupGroupSection(
                        ...
                        onTapPhoto: { asset, category in
                            ...
                            // Compute reason for the tapped photo
                        ...
                        if count > 0 {
                            ...
                            if !appState.isPurchased && appState.freeQuotaProgress > 0.8 {
                                Text(appState.freeQuotaProgress > 0.95
                                     ? "⚠️ Free limit nearly reached — upgrade to keep cleaning"
                                     : "⚠️ \(Int((1 - appState.freeQuotaProgress) * 100))% free quota remaining")
        ...
        Task {
            do {
                ...
                await MainActor.run {
                    ...
                    toastMessage = String(localized: "\(count) duplicate deleted")
...
private struct DupGroupSection: View {
...
private struct DupGridCell: View {
    let asset: PHAsset
    let isSelected: Bool
    let isBest: Bool
    let onSelect: () -> Void
